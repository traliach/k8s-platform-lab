...
- `docs/architecture.png` — rendered PNG architecture diagram (source: `docs/architecture.mmd`)
...
- `scripts/verify-cluster.sh` — end-to-end health check script: 21 checks covering node, ServiceLB, Traefik, namespaces, ArgoCD apps, sample app endpoints, Prometheus targets, Grafana dashboard ConfigMap, and prom-client metrics registration

### Fixed
- `lint.yml` manifest validation: replaced `kubectl apply --dry-run=client` (requires live cluster for resource discovery) with `kubeconform` schema-based validation — no cluster needed in CI
- `release.yml` workflow: moved Docker credential check out of step `if` conditions (secrets context not available there) into a dedicated `docker_check` step writing to `GITHUB_OUTPUT`; Docker build+push now runs correctly when `DOCKER_USERNAME`/`DOCKER_PASSWORD` secrets are set
...
- `scripts/verify-cluster.sh` metrics probe: replaced BusyBox `wget` (unreliable in minimal Alpine containers) with `node -e` HTTP client — guaranteed available in `node:20-alpine` image; probes `127.0.0.1` directly to avoid IPv6 ambiguity
